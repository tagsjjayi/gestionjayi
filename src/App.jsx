...
// ⚠️ AQUÍ DEBES ASEGURARTE DE QUE ESTÉN TUS DATOS REALES
const firebaseConfig = {
  // Pega aquí los datos de tu consola de Firebase si no son estos
...
// Definimos el ID de la app fijo para evitar errores de variables externas
const appId = 'business-admin-system';

// TODO: CAMBIA ESTO POR TU CORREO REAL SI QUIERES VER EL PANEL DE ADMIN (LICENCIAS)
const ADMIN_EMAIL = 'admin@negocio.com'; 
...
const LicenseGate = ({ user, isLicenseActive, onActivate, loadingCheck, children }) => {
  ...
  // Si tiene licencia activa o es modo Demo (Anónimo) o aún estamos verificando, pasar o esperar
  ...
  // CORRECCIÓN: Permitimos acceso si es anónimo (demo) O si tiene licencia activa
  if (isLicenseActive || user?.isAnonymous) {
    return children;
  }
  ...
  const handleActivation = async (e) => {
    ...
    try {
      // 1. Verificar si la licencia existe en la colección pública de licencias
      ...
      if (licenseSnap.exists()) {
        ...
        if (licenseData.status === 'active' || licenseData.status === 'generated') {
           // Licencia válida -> Activar usuario
        ...
        // Fallback para demo si no hay base de datos real configurada aun
    ...
    <div className="min-h-screen bg-slate-900 flex items-center justify-center p-4">
      <div className="bg-white max-w-md w-full rounded-2xl shadow-2xl overflow-hidden">
        <div className="bg-blue-600 p-8 text-center">
          ...
          <p className="text-blue-100 mt-2 text-sm">Esta copia requiere activación.</p>
        </div>
        
        <div className="p-8">
          <p className="text-slate-600 text-center mb-6 text-sm">
            Hola, <strong>{user?.email || 'Usuario'}</strong>. Ingrese su licencia de compra para desbloquear el sistema.
          </p>
          ...
          <div className="mt-6 text-center pt-6 border-t border-slate-100">
            <div className="mt-4">
              <button onClick={() => signOut(auth)} className="text-slate-400 text-xs hover:text-slate-600 flex items-center justify-center gap-1 mx-auto">
                <LogOut className="w-3 h-3"/> Cambiar Cuenta / Cerrar Sesión
              </button>
            </div>
          </div>
        </div>
...
export default function App() {
  ...
  const [adminLicenses, setAdminLicenses] = useState([]); // Para el panel de admin
  ...
  // --- Autenticación (CORREGIDA PARA MANUAL) ---
  useEffect(() => {
    const initAuth = async () => {
      try {
        // Intentamos iniciar sesión (verificamos si ya hay usuario)
        // Si no hay usuario, el onAuthStateChanged se disparará como null
        // y ahí podemos decidir si forzar un login anónimo
        setAuthError(null);
      } catch (error) {
    ...
    const unsubscribe = onAuthStateChanged(auth, async (currentUser) => {
      if (!currentUser) {
         // Si no hay usuario logueado, iniciamos anónimo automáticamente
         try {
            await signInAnonymously(auth);
         } catch (e) {
            console.error("Error login anonimo", e);
      ...
      if (currentUser) {
        // Verificar estado de licencia en DB personal del usuario
        if (currentUser.isAnonymous) {
          setIsLicenseActive(true); // Demo siempre activa
  ...
  useEffect(() => {
    if (!user || (!isLicenseActive && !user.isAnonymous)) {
    ...
    const unsubCred = onSnapshot(collection(db, 'artifacts', appId, 'users', userId, 'credits'), (s) => { setCredits(s.docs.map(d => ({ id: d.id, ...d.data() }))); setLoading(false); });
    
    // Si soy admin, cargar licencias
    let unsubLicenses = () => {};
  ...
  // --- Función de Activación de Licencia ---
  ...
  // --- Funciones de Administrador (Generar Licencias) ---
  const generateNewLicense = async () => {
    if (user?.email !== ADMIN_EMAIL) return;
    // Generar string aleatorio simple de 8 caracteres
    ...
    try {
      ...
      console.error(e);
      showNotification("Error al crear licencia", "error");
    }
  };
  ...
  const handleAddDoc = async (e, colName, data) => {
    ...
    if (!user) return;
    try {
      await addDoc(collection(db, 'artifacts', appId, 'users', user.uid, colName), { ...data, createdAt: new Date().toISOString() });
      e.target.reset();
  ...
  const handleSmartTransactionSubmit = async (e) => {
    ...
    if (selectedTxProduct) {
      ...
      try {
        ...
        showNotification(`Venta registrada: $${precioFinal.toLocaleString()}`);
        setTxSearchTerm(''); setSelectedTxProduct(null); setTxQty(1); setIsBoxTx(false); form.reset();
      ...
      try {
        ...
        showNotification("Movimiento registrado"); form.reset();
      } catch (err) { console.error(err); }
    }
  };
  ...
  const handleInventorySubmit = async (e) => {
    ...
    if (existingProduct) {
      ...
      try {
        await updateDoc(doc(db, 'artifacts', appId, 'users', user.uid, 'inventory', existingProduct.id), {
        ...
        }); showNotification(`Stock actualizado: +${unidadesAAgregar} unidades`);
      } catch (err) { console.error(err); }
    } else {
      ...
      try {
        ...
        }); showNotification("Producto registrado");
      } catch (err) { console.error(err); }
    }
  ...
  // LOGIN CON GOOGLE SIMULADO O REAL
  const handleGoogleLogin = async () => { 
    // Para StackBlitz/Manual, lo ideal es usar Popup si está configurado en Firebase, 
    // pero si no, dejamos el modal informativo.
    try {
       const provider = new GoogleAuthProvider();
       await signInWithPopup(auth, provider);
    } catch(e) {
       console.log("Login popup cerrado o no configurado", e);
  ...
  if (loading || checkingLicense) return <div className="min-h-screen flex items-center justify-center bg-slate-50"><RefreshCcw className="w-8 h-8 animate-spin text-blue-600"/></div>;
  if (authError) return <div className="p-10 text-center text-red-500">{authError}</div>;
  ...
  return (
    <LicenseGate user={user} isLicenseActive={isLicenseActive} onActivate={activateLicense} loadingCheck={checkingLicense}>
      <div className="min-h-screen bg-slate-50 flex flex-col lg:flex-row font-sans text-slate-900">
        ...
        <Modal isOpen={showAuthModal} onClose={() => setShowAuthModal(false)} title="Función Premium: Google Login">
          <div className="text-center space-y-4">
            <div className="w-16 h-16 bg-blue-50 rounded-full flex items-center justify-center mx-auto"><ShieldCheck className="w-8 h-8 text-blue-600" /></div>
            <p className="text-slate-600">Para activar el login con Google real, asegúrate de habilitar el proveedor "Google" en tu consola de Firebase Authentication.</p>
            <Button onClick={() => setShowAuthModal(false)} className="w-full">Entendido</Button>
          </div>
        </Modal>
        ...
        <nav className="w-full lg:w-72 bg-slate-900 text-white p-6 flex flex-col gap-8 shadow-xl z-20">
          ...
          <div className="mt-auto space-y-4 pt-6 border-t border-slate-800">
            ...
            {user?.isAnonymous && (
              <button onClick={handleGoogleLogin} className="w-full flex items-center gap-3 p-3 rounded-xl bg-slate-800 hover:bg-slate-700 transition-colors group">
                ...
                <div className="text-left overflow-hidden"><p className="text-xs font-bold text-white truncate">Vincular Cuenta</p><p className="text-[10px] text-slate-400 truncate">Guardar en la nube</p></div>
              </button>
            )}
            <button onClick={() => { if(window.confirm("¿Reiniciar Demo?")) resetAllData(); }} className="w-full flex items-center justify-center gap-2 p-2 text-xs text-red-400 hover:text-red-300 transition-colors">
              <Database className="w-3 h-3" /> Reiniciar Demo
            </button>
          </div>
        </nav>
        ...
        <main className="flex-1 p-6 lg:p-10 overflow-auto max-h-screen">
          <header className="mb-8 flex justify-between items-end">
            <div>
              ...
              <p className="text-slate-500 text-sm mt-1">{user?.isAnonymous ? "Modo Demo" : `Bienvenido, ${user?.email}`}</p>
            </div>
          </header>
              ...
              <div className="space-y-6">
                <Card className="p-6 border-amber-200 border-2">
                  <div className="flex justify-between items-center mb-6">
                    <div>
                      <h3 className="text-xl font-bold text-slate-800 flex items-center gap-2"><Crown className="w-6 h-6 text-amber-500"/> Generador de Licencias</h3>
                      <p className="text-slate-500 text-sm">Crea códigos para vender el acceso a tu aplicación.</p>
                    </div>
                    <Button onClick={generateNewLicense} variant="primary" className="bg-amber-500 hover:bg-amber-600 border-none shadow-lg shadow-amber-200">
                      <Plus className="w-5 h-5"/> Generar Nueva Licencia
                    </Button>
                  </div>
                      ...
                      <tbody className="divide-y divide-slate-100">
                        {adminLicenses.map(lic => (
                          <tr key={lic.id} className="hover:bg-slate-50">
                            <td className="p-4 font-mono font-bold text-lg text-blue-600 select-all">{lic.code}</td>
                            <td className="p-4"><span className="px-3 py-1 bg-emerald-100 text-emerald-700 rounded-full text-xs font-bold uppercase">{lic.status}</span></td>
                            <td className="p-4 text-slate-500 text-sm">{new Date(lic.createdAt).toLocaleDateString()}</td>
                          </tr>
                        ))}
                        {adminLicenses.length === 0 && (
                          <tr><td colSpan="3" className="p-8 text-center text-slate-400">Aún no has generado ninguna licencia.</td></tr>
                        )}
                      </tbody>
                    </table>
                  </div>
                ...
                <Card className="p-6 border-blue-200 shadow-md">
                  ...
                  <form onSubmit={handleSmartTransactionSubmit} className="grid grid-cols-1 md:grid-cols-12 gap-4">
                    ...
                    <div className="md:col-span-3 space-y-4">
                        <Input name="fecha" type="date" label="Fecha" defaultValue={new Date().toISOString().split('T')[0]} required />
                        
                        {/* Buscador de Producto */}
                        <div className="relative">
                          <label className="text-sm font-semibold text-slate-700">Buscar Producto (Opcional)</label>
                          <div className="relative mt-1">
                            ...
                            <input 
                              ...
                              onChange={(e) => {
                                ...
                                if(!e.target.value) setSelectedTxProduct(null);
                              }}
                              className="w-full pl-9 pr-3 py-2 bg-white border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:outline-none"
                            />
                          </div>
                          {txSearchTerm && !selectedTxProduct && filteredProducts.length > 0 && (
                            <div className="absolute z-10 w-full bg-white border border-slate-200 rounded-lg shadow-xl mt-1 overflow-hidden">
                              {filteredProducts.map(p => (
                                <div key={p.id} onClick={() => { setSelectedTxProduct(p); setTxSearchTerm(p.articulo); }} className="p-2 hover:bg-blue-50 cursor-pointer text-sm flex justify-between items-center">
                                  <span>{p.articulo}</span><span className="font-mono text-xs text-slate-400">{p.codigo}</span>
                                </div>
                              ))}
                            </div>
                          )}
                    ...
                    {/* Columna Central: Detalles Dinámicos */}
                    <div className="md:col-span-6 bg-slate-50 p-4 rounded-xl border border-slate-200 flex flex-col justify-center">
                        {selectedTxProduct ? (
                          <div className="space-y-3 animate-in fade-in">
                            <div className="flex justify-between items-start">
                              <div><h4 className="font-bold text-slate-800">{selectedTxProduct.articulo}</h4><p className="text-xs text-slate-500">Stock actual: {selectedTxProduct.inicial + (selectedTxProduct.entradas||0) - (selectedTxProduct.salidas||0)} un.</p></div>
                              <button onClick={()=>{setSelectedTxProduct(null); setTxSearchTerm('');}} className="text-xs text-red-400 hover:text-red-600">Cambiar</button>
                            </div>
                            <div className="flex gap-4 items-end">
                              <div className="w-24"><label className="text-xs font-bold text-slate-500">Cantidad</label><input type="number" min="1" value={txQty} onChange={(e)=>setTxQty(Number(e.target.value))} className="w-full px-2 py-1 border rounded font-bold text-center"/></div>
                              {selectedTxProduct.esCaja && (
                                <div className="flex-1"><label className="text-xs font-bold text-slate-500 block mb-1">Unidad de Venta</label><div className="flex gap-2 text-xs"><button type="button" onClick={()=>setIsBoxTx(false)} className={`flex-1 py-1 rounded border ${!isBoxTx ? 'bg-blue-600 text-white border-blue-600' : 'bg-white text-slate-500'}`}>Unidad</button><button type="button" onClick={()=>setIsBoxTx(true)} className={`flex-1 py-1 rounded border ${isBoxTx ? 'bg-blue-600 text-white border-blue-600' : 'bg-white text-slate-500'}`}>Caja</button></div></div>
                              )}
                            </div>
                          </div>
                        ) : (
                          <div className="space-y-4">
                            <Input name="concepto" label="Concepto / Detalle" placeholder="Ej: Pago de Luz, Venta Varia..." />
                            <div className="text-xs text-slate-400 text-center italic mt-2">Seleccione un producto para autocompletar venta de inventario.</div>
                          </div>
                        )}
                    </div>

                    {/* Columna Derecha: Totales y Acción */}
                    <div className="md:col-span-3 flex flex-col justify-between space-y-4">
                        {selectedTxProduct ? (
                          <div className="bg-emerald-50 p-3 rounded-lg border border-emerald-100 text-center">
                            <p className="text-xs text-emerald-600 font-bold uppercase">Total a Ingresar</p>
                            <p className="text-2xl font-black text-emerald-700">
                              ${(() => {
                                  const p = selectedTxProduct; const q = txQty;
                                  if (isBoxTx && p.esCaja) return (p.precioVenta * q).toLocaleString();
                                  if (p.esCaja) return ((p.precioVenta / p.unidades) * q).toLocaleString(undefined, {minimumFractionDigits: 0, maximumFractionDigits: 2});
                                  return (p.precioVenta * q).toLocaleString();
                              })()}
                            </p>
                          </div>
                        ) : (
                          <div className="grid grid-cols-2 gap-2">
                            <Input name="ingresos" type="number" label="Ingreso ($)" placeholder="0.00" />
                            <Input name="egresos" type="number" label="Egreso ($)" placeholder="0.00" />
                          </div>
                        )}
                        <Button type="submit" className="w-full h-12 shadow-lg shadow-blue-100">{selectedTxProduct ? <><ShoppingBag className="w-4 h-4"/> Confirmar Venta</> : <><Plus className="w-4 h-4"/> Registrar</>}</Button>
                    </div>
                  </form>
                </Card>
                <Card>
                  <table className="w-full text-left text-sm">
                    <thead className="bg-slate-50 text-slate-700"><tr><th className="p-3">Fecha</th><th className="p-3">Concepto</th><th className="p-3">Monto</th><th className="p-3 text-right">Acción</th></tr></thead>
                    <tbody className="divide-y">{transactions.map(t => (<tr key={t.id}><td className="p-3">{t.fecha}</td><td className="p-3 font-medium">{t.concepto}</td><td className={`p-3 font-bold ${t.ingresos>0?'text-emerald-600':'text-red-600'}`}>{t.ingresos>0?`+${t.ingresos.toLocaleString()}`:`-${t.egresos.toLocaleString()}`}</td><td className="p-3 text-right"><button onClick={()=>deleteItem('transactions', t.id)} className="text-red-300 hover:text-red-500"><Trash2 className="w-4 h-4"/></button></td></tr>))}</tbody>
                  </table>
                </Card>
              </div>
            )}
            ...
            {activeTab === 'inventory' && (
              <div className="space-y-6">
                <Card className="p-6 border-blue-200">
                  <div className="flex items-center gap-2 mb-4 text-blue-800"><ScanBarcode className="w-5 h-5"/><h3 className="font-bold">Registro Inteligente de Producto</h3></div>
                  ...
                  <form onSubmit={handleInventorySubmit} className="space-y-6">
                    <div className="grid md:grid-cols-4 gap-4">
                      <Input name="codigo" label="Código (Escáner/Manual)" placeholder="Ej: 770..." required autoFocus />
                      <div className="md:col-span-2"><Input name="articulo" label="Nombre del Producto" placeholder="Automático si ya existe" /></div>
                      <Input name="proveedor" label="Proveedor" placeholder="Opcional" />
                    </div>
                    <div className="grid md:grid-cols-3 gap-4 bg-slate-50 p-4 rounded-xl border border-slate-200">
                      <div><label className="text-sm font-semibold text-slate-700 block mb-1">Costo ($)</label><input type="number" value={invForm.costo} onChange={(e) => setInvForm({...invForm, costo: Number(e.target.value)})} className="w-full px-3 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none" placeholder="0.00" /></div>
                      <div><label className="text-sm font-semibold text-slate-700 block mb-1">Ganancia (%)</label><div className="flex items-center"><input type="number" value={invForm.margen} onChange={(e) => setInvForm({...invForm, margen: Number(e.target.value)})} className="w-full px-3 py-2 border border-slate-300 rounded-l-lg focus:ring-2 focus:ring-blue-500 outline-none text-center" /><span className="bg-slate-200 px-3 py-2 rounded-r-lg border border-l-0 border-slate-300 text-slate-600 font-bold">%</span></div></div>
                      <div><label className="text-sm font-semibold text-emerald-700 block mb-1">Precio Venta (Calculado)</label><div className="px-3 py-2 bg-emerald-50 border border-emerald-200 rounded-lg text-emerald-800 font-bold text-lg">${precioVentaCalculado.toLocaleString(undefined, {minimumFractionDigits: 0, maximumFractionDigits: 2})}</div></div>
                    </div>
                    <div className="flex flex-col md:flex-row gap-4 items-start md:items-center p-3 rounded-lg border border-dashed border-slate-300">
                      <label className="flex items-center gap-2 cursor-pointer select-none"><input type="checkbox" checked={invForm.esCaja} onChange={(e) => setInvForm({...invForm, esCaja: e.target.checked})} className="w-5 h-5 text-blue-600 rounded focus:ring-blue-500" /><span className="font-medium text-slate-700 flex gap-1 items-center"><Box className="w-4 h-4"/> ¿Es Caja/Bulto?</span></label>
                      {invForm.esCaja && (<><div className="flex items-center gap-2 animate-in slide-in-from-left-5"><span className="text-sm text-slate-500">Contiene:</span><input type="number" value={invForm.unidades} onChange={(e) => setInvForm({...invForm, unidades: Number(e.target.value)})} className="w-20 px-2 py-1 border border-slate-300 rounded text-center font-bold" /><span className="text-sm text-slate-500">unidades</span></div><div className="flex-1 text-xs md:text-sm text-slate-500 bg-yellow-50 p-2 rounded border border-yellow-100">💡 Unitario: Costo <strong>${unitariosCalculados?.costoUnitario.toFixed(2)}</strong> | Venta <strong>${unitariosCalculados?.precioUnitario.toFixed(2)}</strong></div></>)}
                    </div>
                    <div className="grid md:grid-cols-2 gap-4"><Input name="inicial" type="number" label={invForm.esCaja ? "Cantidad de CAJAS a agregar" : "Cantidad de UNIDADES a agregar"} required placeholder="0" /><Button type="submit" className="h-full mt-auto"><Plus className="w-4 h-4"/> Registrar / Reponer</Button></div>
                  </form>
                </Card>
                <Card className="overflow-hidden">
                  <div className="overflow-x-auto">
                  <table className="w-full text-left text-sm">
                    <thead className="bg-slate-50 text-slate-700"><tr><th className="p-3">Cód</th><th className="p-3">Producto</th><th className="p-3 text-right">Precio Venta</th><th className="p-3 text-center">Stock Real</th><th className="p-3 text-right">Acciones</th></tr></thead>
                    <tbody className="divide-y">
                      {inventory.map(i => {
                        const stockTotal = i.inicial + (i.entradas||0) - (i.salidas||0); const cajasAprox = i.esCaja ? Math.floor(stockTotal / i.unidades) : 0; const unidadesSueltas = i.esCaja ? stockTotal % i.unidades : 0;
                        return (<tr key={i.id} className="hover:bg-slate-50"><td className="p-3 font-mono text-slate-500">{i.codigo}</td><td className="p-3"><div className="font-bold text-slate-800">{i.articulo}</div>{i.esCaja && <div className="text-[10px] text-blue-500 bg-blue-50 inline-block px-1 rounded border border-blue-100">Caja de {i.unidades} un.</div>}</td><td className="p-3 text-right font-bold text-emerald-700">${(i.precioVenta || 0).toLocaleString()}{i.esCaja && <div className="text-[10px] text-slate-400 font-normal">(${(i.precioVenta/i.unidades).toFixed(2)} un.)</div>}</td><td className="p-3 text-center"><span className={`px-2 py-1 rounded-full font-bold ${stockTotal<5?'bg-red-100 text-red-700':'bg-blue-50 text-blue-700'}`}>{stockTotal} un.</span>{i.esCaja && <div className="text-[10px] text-slate-400 mt-1">{cajasAprox} Cajas + {unidadesSueltas} sueltas</div>}</td><td className="p-3 text-right flex justify-end gap-2"><button onClick={()=>deleteItem('inventory', i.id)} className="text-red-300 hover:text-red-500 p-2"><Trash2 className="w-4 h-4"/></button></td></tr>);
                      })}
                    </tbody>
                  </table>
                  </div>
            ...
            {activeTab === 'customers' && (
              <div className="space-y-6">
                <Card className="p-6">
                  ...
                  <form onSubmit={(e) => handleAddDoc(e, 'customers', { nit: e.target.nit.value, nombre: e.target.nombre.value, tel: e.target.tel.value, email: e.target.email.value, ciudad: e.target.ciudad.value, direccion: e.target.direccion.value, descripcion: e.target.descripcion.value, notas: e.target.notas.value })} className="grid md:grid-cols-3 gap-4">
                    <Input name="nit" label="ID/NIT" required /><Input name="nombre" label="Nombre" required /><Input name="tel" label="Teléfono" /><Input name="email" label="Email" /><Input name="ciudad" label="Ciudad" /><Input name="direccion" label="Dirección" />
                    <div className="md:col-span-3 grid md:grid-cols-2 gap-4"><Input name="descripcion" label="Descripción del Cliente" placeholder="Ej: Mayorista, Minorista, VIP..." /><Input name="notas" label="Notas / Observaciones" placeholder="Preferencias, horarios, etc." /></div>
                    <div className="md:col-span-3"><Button type="submit"><Plus className="w-4 h-4"/> Guardar Cliente</Button></div>
                  </form>
                </Card>
                <Card>
                  <table className="w-full text-left text-sm">
                    <thead className="bg-slate-50 text-slate-700"><tr><th className="p-3">Nombre / Detalle</th><th className="p-3">Contacto</th><th className="p-3">Ubicación</th><th className="p-3 text-right">Acción</th></tr></thead>
                    <tbody className="divide-y">
                      {customers.map(c => (
                        <tr key={c.id}><td className="p-3"><div className="font-bold text-slate-800">{c.nombre}</div><div className="text-xs text-slate-500 font-mono">ID: {c.nit}</div>{c.descripcion && <div className="text-xs text-blue-600 mt-1 font-medium">{c.descripcion}</div>}{c.notas && <div className="text-xs text-slate-500 italic mt-0.5 bg-yellow-50 p-1 rounded border border-yellow-100 flex items-center gap-1 w-fit"><StickyNote className="w-3 h-3"/> {c.notas}</div>}</td><td className="p-3 text-slate-600"><div>{c.tel}</div><div className="text-xs">{c.email}</div></td><td className="p-3 text-slate-600"><div>{c.ciudad}</div><div className="text-xs">{c.direccion}</div></td><td className="p-3 text-right"><button onClick={()=>deleteItem('customers', c.id)} className="text-red-300 hover:text-red-500"><Trash2 className="w-4 h-4"/></button></td></tr>
                      ))}
                    </tbody>
                  </table>
                </Card>
